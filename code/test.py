...
for code in name:
    ...
    X_lstm_2d = X_lstm.reshape(X_lstm.shape[0], X_lstm.shape[1])
    df = pd.DataFrame(X_lstm_2d, columns=[f'feature_{i}' for i in range(X_lstm_2d.shape[1])])
    df['true_var'] = y_true_var
    df['garch_var'] = y_garch_var

    df.to_csv('dataset.csv', index=False, mode = 'a')
...
# print(f"Shape of y_true_var: {y_true_var.shape}")
# print(f"Shape of y_garch_var: {y_garch_var.shape}")
# print(X_lstm)
