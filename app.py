...
DB_CONFIG = {
    ...
    "port": 3306,
    "user": "root",
    "password": "5333454275asd",  # 请根据实际情况填写密码
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                sql = "SELECT * FROM User WHERE user_name = %s AND user_status = 'active'"
                ...
                # 更新最后登录时间
                update_sql = "UPDATE User SET user_last_login_time = NOW() WHERE user_id = %s"
                cursor.execute(update_sql, (user['user_id'],))
                connection.commit()
                
                # 生成token
                token = generate_token(user['user_id'], user['user_name'], user['user_role'])
...
def get_params(current_user):
    ...
    try:
        ...
        param_type_filter = request.args.get('param_type', 'my')  # my, system, public, all
        param_source_type = request.args.get('param_source_type', 'all')  # all, table, indicator
            ...
            with connection.cursor() as cursor:
                ...
                if indicator_relations or strategy_relations:
                    ...
                    if indicator_relations:
                        indicator_names = [rel['indicator_name'] for rel in indicator_relations]
                        usage_info.append(f"指标: {', '.join(indicator_names)}")
                    if strategy_relations:
                        strategy_names = [rel['strategy_name'] for rel in strategy_relations]
...
def get_data_sources(current_user):
    ...
    try:
        ...
        data_sources = [
            ...
            'daily.volume', 'daily.amount', 'daily.pct_change',
            'daily_basic.total_mv', 'daily_basic.circ_mv', 'daily_basic.pe', 
            'daily_basic.pb', 'daily_basic.ps', 'daily_basic.pcf',
            ...
            # 技术指标
            'indicator.MACD', 'indicator.KDJ', 'indicator.RSI', 
            'indicator.BOLL', 'indicator.MA', 'indicator.EMA',
            'indicator.VOLMA', 'indicator.ATR', 'indicator.CCI',
            
            # 系统指标（来自Indicator表）
            'system.MACD', 'system.KDJ', 'system.RSI'
        ]
        ...
        # 过滤数据源
        if query:
            filtered_sources = [source for source in data_sources if query in source.lower()]
        else:
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                # 根据搜索关键词筛选
                if search_keyword:
                    where_conditions.append("(indicator_name LIKE %s OR description LIKE %s)")
                    query_params.extend([f'%{search_keyword}%', f'%{search_keyword}%'])
...
def create_indicator(current_user):
    ...
    try:
        ...
        indicator_name = str(data['indicator_name']).strip() if data['indicator_name'] is not None else ''
        calculation_method = str(data['calculation_method']).strip() if data['calculation_method'] is not None else ''
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if cursor.fetchone():
                    return jsonify({'message': f'指标名称 "{indicator_name}" 已存在'}), 400
...
def update_indicator(current_user, indicator_composite_id):
    ...
    try:
        ...
        new_indicator_name = str(data['indicator_name']).strip() if data['indicator_name'] is not None else ''
        calculation_method = str(data['calculation_method']).strip() if data['calculation_method'] is not None else ''
        description = str(data.get('description', '')).strip() if data.get('description') else None
        is_active = bool(data.get('is_active', True))
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if new_indicator_name != indicator_name:
                    ...
                    cursor.execute(check_new_name_sql, (creator_name, new_indicator_name))
                    if cursor.fetchone():
                        return jsonify({'message': f'新的指标名称 "{new_indicator_name}" 已存在'}), 400
                ...
                if new_indicator_name != indicator_name:
                    ...
                    """
                    cursor.execute(update_indicator_rel_sql, (new_indicator_name, creator_name, indicator_name))
                    ...
                    """
                    cursor.execute(update_sql, (
                        new_indicator_name, calculation_method, description, is_active,
                    ...
                    """
                    cursor.execute(update_sql, (
                        calculation_method, description, is_active,
                        creator_name, indicator_name
                    ))
                ...
                updated_indicator = {
                    ...
                    'description': description,
                    'is_active': is_active,
                    'create_time': existing_indicator.get('create_time', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                }
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                """
                cursor.execute(check_param_rel_sql, (f'{creator_name}.{indicator_name}%',))
...
def create_indicator_param_relation(current_user):
    ...
    try:
        ...
        # 验证必需字段
        required_fields = ['indicator_creator_name', 'indicator_name', 'param_creator_name', 'param_name']
        for field in required_fields:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(indicator_check_sql, (indicator_creator_name, indicator_name))
                if not cursor.fetchone():
                    return jsonify({'message': '指定的指标不存在'}), 404
                
                # 验证参数是否存在
                param_check_sql = "SELECT * FROM Param WHERE creator_name = %s AND param_name = %s"
                ...
                """
                cursor.execute(check_sql, (indicator_creator_name, indicator_name, 
                ...
                VALUES (%s, %s, %s, %s)
                """
                cursor.execute(insert_sql, (indicator_creator_name, indicator_name,
                                          param_creator_name, param_name))
                connection.commit()
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                AND param_creator_name = %s AND param_name = %s
                """
                cursor.execute(delete_sql, (indicator_creator_name, indicator_name,
                                          param_creator_name, param_name))
                connection.commit()
