...
CREATE TABLE IndicatorParamRel (
    ...
    CONSTRAINT fk_rel_indicator FOREIGN KEY (
        indicator_creator_name,
        indicator_name
    ) REFERENCES Indicator (creator_name, indicator_name),
    CONSTRAINT fk_rel_param_indicator FOREIGN KEY (
        param_creator_name,
        param_name
    ) REFERENCES Param (creator_name, param_name)
...
CREATE TABLE StrategyParamRel (
    ...
    CONSTRAINT fk_rel_strategy FOREIGN KEY (
        strategy_creator_name,
        strategy_name
    ) REFERENCES Strategy (creator_name, strategy_name),
    CONSTRAINT fk_rel_param_strategy FOREIGN KEY (
        param_creator_name,
        param_name
    ) REFERENCES Param (creator_name, param_name)
...
CREATE TABLE BacktestReport (
    ...
    report_status ENUM(
        ...
        'completed',
        'failed'
    ) NOT NULL DEFAULT 'generating' COMMENT '报告状态',
    error_message TEXT COMMENT '当报告状态为failed时存储错误信息',
    PRIMARY KEY (report_id),
    INDEX idx_strategy_generate_time (
        creator_name,
        strategy_name,
    ...
    CONSTRAINT chk_funds CHECK (
        initial_fund > 0
        AND final_fund >= 0
    ),
    CONSTRAINT chk_drawdown CHECK (max_drawdown >= 0),
    CONSTRAINT chk_win_rate CHECK (
        win_rate IS NULL
        OR win_rate BETWEEN 0 AND 1
    ),
    CONSTRAINT chk_ratio CHECK (
        profit_loss_ratio IS NULL
        OR profit_loss_ratio >= 0
    ),
