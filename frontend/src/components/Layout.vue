  ...
  <div class="app-container">
    ...
    >
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="用户角色">{{ userInfo?.user_role === 'admin' ? '管理员' : '分析师' }}</el-descriptions-item>
        <el-descriptions-item label="用户状态">{{ userInfo?.user_status }}</el-descriptions-item>
        <el-descriptions-item label="邮箱" v-if="userInfo?.user_email">{{ userInfo?.user_email }}</el-descriptions-item>
        <el-descriptions-item label="手机号" v-if="userInfo?.user_phone">{{ userInfo?.user_phone }}</el-descriptions-item>
        <el-descriptions-item label="创建时间">{{ formatDate(userInfo?.user_create_time) }}</el-descriptions-item>
        <el-descriptions-item label="最后登录时间">{{ formatDate(userInfo?.user_last_login_time) }}</el-descriptions-item>
      </el-descriptions>
      <template #footer>
